...
/**
 * Class for the hoof action. Average dmg.
 */
public class Hoof extends Action {

    @Override
    public double perform(Chara source, Chara target, double previous, List<Chara> team) {

        double dmg = Random.randDouble(source.getAttack() - (source.getAttack() / 3.0), source.getAttack() + (source.getAttack() / 5.0)) - (target.getDefense() / 10.0);
